<template>
  <section class="position-relative overflow-hidden pt-5 pt-lg-4">
  <div class="container">
    <!-- Title -->
        ...
        <div class="row g-4">
          ...
          <div class="col-6" v-for="(item, index) in items" :key="index">
            <div class="d-sm-flex align-items-center">
              <div :class="['icon-md', 'fs-5', 'text-white', item.bgClass, 'rounded']">
                <i :class="item.icon"></i>
              </div>
              <div class="ms-0 ms-sm-3 mt-2 mt-sm-0">
                <h6 class="mb-0">{{ item.title }}</h6>
                <div class="small"><i class="bi bi-stopwatch me-2"></i>{{ item.subtitle }}</div>
              </div>
            </div>
          </div>
        </div> <!-- Row END -->
